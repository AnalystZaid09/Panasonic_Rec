...
def main():
    ...
    st.title('PDF ⇄ Excel Reconciliation (Azure Document Intelligence required)')
    st.markdown('Upload invoice PDF(s) and reference Excel. SKU matching prefers model prefix at start of description (e.g. SR-WA18H).')
    ...
    if st.button('Run Reconciliation'):
        ...
        for pdf_file in uploaded_pdf:
            ...
            try:
                ...
                towrite.seek(0)
                bname = f"recon_{os.path.splitext(pdf_file.name)[0]}.xlsx"
                st.download_button(
